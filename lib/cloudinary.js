...
async function uploadToCloudinary(source, folder, publicId) {
  console.log('[DEBUG Cloudinary] Upload called with:', {
    sourceType: Buffer.isBuffer(source) ? 'Buffer' : typeof source,
    sourceLength: Buffer.isBuffer(source) ? source.length : source?.length,
    folder,
    publicId
  });
  ...
  if (Buffer.isBuffer(source)) {
    console.log('[DEBUG Cloudinary] Uploading buffer...');
    const result = await uploadBuffer(source, options);
    console.log('[DEBUG Cloudinary] Upload successful:', result);
    return {
  ...
  if (typeof source === 'string') {
    ...
    if (/^https?:\/\//i.test(trimmed)) {
      console.log('[DEBUG Cloudinary] Uploading from URL...');
    ...
    if (trimmed.startsWith('data:') || /^[A-Za-z0-9+/]+=*$/.test(trimmed)) {
      console.log('[DEBUG Cloudinary] Uploading from base64...');
